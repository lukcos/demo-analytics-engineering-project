...
I took this model and created some ETL scripts that transform the JSON objects and load them into a postgres database. All of the scripts follow the basic structure of loading in the data with a basic `load_json_data()` function I've defined. From there, the `extract_data()` function is script and JSON object dependent. My approach was to iterate over the list of records provided in the JSON, create a new dict from each record where I extract key values stored in nested JSON fields - ie how most ids are stored as `json._id.$0id`. Parsing out that `$oid` part and just using that as our main `id` - aside from creating `rewards_receipt_items`. There's a little bit of cleaning that happens, popping some of the values from json that either are redundant or get their own table, and then load that into a polars lazyframe. Using a lazyframe means I can keep any additional transformations computationally cheap and fast. `process_{json}()` takes that lazyframe and then refines it to have the correct datatypes so that we can load it into our postgres database. For future development of this, I'd use this function to introduce any normalization and sanitization of the data to make it more consistent -> ie flagging certain vals in fields that we know are erroneous, standardizing format of tex, introducing dict to value maps for multiple values that are known to be the same, etc. Then `load_data()` is your standard load function using polards `df.write_database()`. Modifications here would be to not truncate the data on every load, but create an upsert function to only update new vals. This would mean introducing a new schema, new fields for `db_last_modified` values that could be used to make this even more performative. All of these scripts could then be used in a DAG flow with dbt models for further transformation.
